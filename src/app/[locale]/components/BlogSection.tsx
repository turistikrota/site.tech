export default function BlogSection() {
  return (
    <section className='py-20'>
      <div className='container'>
        <div className='text-center'>
          <span className='rounded-full bg-primary/10 px-3 py-1 text-sm font-medium text-primary'>Blog</span>
          <h1 className='my-3 text-3xl font-medium'>Interesting Articles</h1>
        </div>
        <div className='mt-12 grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3'>
          <div className='aos-init aos-animate rounded-md shadow-md' data-aos='fade-up' data-aos-duration='500'>
            <div className='relative'>
              <div className='absolute end-4 top-3'>
                <span className='rounded-md bg-black px-3 py-1 text-sm font-medium text-white'>Design</span>
              </div>
              <img src='/prompt_tr/assets/coworking1-47a0c6a8.jpg' />
              <div className='absolute -bottom-5'>
                <svg
                  className='h-14 w-full text-white'
                  viewBox='0 0 528 40'
                  version='1.1'
                  xmlnsXlink='http://www.w3.org/1999/xlink'
                  xmlns='http://www.w3.org/2000/svg'
                >
                  <g id='shape' transform='matrix(-1.138336E-07 -1 1 -1.138336E-07 0 39.92764)'>
                    <path
                      d='M0 0L40.5467 0C40.5467 0 -31.8215 230.87 38.7134 528.217C39.8794 533.133 31.7549 527.502 31.0925 528.75C28.7914 533.084 26.1543 528.191 24.4327 529.178C59.2372 539.206 14.0091 521.981 12.9329 530.001L1.02722 528.284L0 0Z'
                      transform='translate(7.629395E-06 6.103516E-05)'
                      fill='currentColor'
                      stroke='none'
                    ></path>
                  </g>
                </svg>
              </div>
            </div>
            <div className='p-6'>
              <p className='text-sm'>11 March, 2020</p>
              <h4 className='my-2 text-lg font-semibold hover:text-blue-700'>
                <a href='/prompt_tr/landing/agency'>Top 10 design inspirations to follow</a>
              </h4>
              <p className='my-2 text-slate-400'>
                Single page websites are taking over the world, and thats why I would like you to present the best
                ...&nbsp;
                <a className='text-slate-800 hover:text-blue-700' href='/prompt_tr/landing/agency'>
                  Read More
                </a>
              </p>
            </div>
          </div>
          <div className='aos-init aos-animate rounded-md shadow-md' data-aos='fade-up' data-aos-duration='500'>
            <div className='relative'>
              <div className='absolute end-4 top-3'>
                <span className='rounded-md bg-primary px-3 py-1 text-sm font-medium text-white'>Development</span>
              </div>
              <img src='/prompt_tr/assets/coworking2-2fcd89b1.png' />
              <div className='absolute -bottom-5'>
                <svg
                  className='h-14 w-full text-white'
                  viewBox='0 0 528 40'
                  version='1.1'
                  xmlnsXlink='http://www.w3.org/1999/xlink'
                  xmlns='http://www.w3.org/2000/svg'
                >
                  <g id='shape' transform='matrix(-1.138336E-07 -1 1 -1.138336E-07 0 39.92764)'>
                    <path
                      d='M0 0L40.5467 0C40.5467 0 -31.8215 230.87 38.7134 528.217C39.8794 533.133 31.7549 527.502 31.0925 528.75C28.7914 533.084 26.1543 528.191 24.4327 529.178C59.2372 539.206 14.0091 521.981 12.9329 530.001L1.02722 528.284L0 0Z'
                      transform='translate(7.629395E-06 6.103516E-05)'
                      fill='currentColor'
                      stroke='none'
                    ></path>
                  </g>
                </svg>
              </div>
            </div>
            <div className='p-6'>
              <p className='text-sm'>12 March, 2020</p>
              <h4 className='my-2 text-lg font-semibold hover:text-blue-700'>
                <a href='/prompt_tr/landing/agency'>Top 10 design inspirations to follow</a>
              </h4>
              <p className='my-2 text-slate-400'>
                We have shortlisted the best WordPress themes for alcohol production, distribution, and selling
                to...&nbsp;
                <a className='text-slate-800 hover:text-blue-700' href='/prompt_tr/landing/agency'>
                  Read More
                </a>
              </p>
            </div>
          </div>
          <div className='aos-init aos-animate rounded-md shadow-md' data-aos='fade-up' data-aos-duration='500'>
            <div className='relative'>
              <div className='absolute end-4 top-3'>
                <span className='rounded-md bg-black px-3 py-1 text-sm font-medium text-white'>Design</span>
              </div>
              <img src='/prompt_tr/assets/coworking4-2154e3ea.jpg' />
              <div className='absolute -bottom-5'>
                <svg
                  className='h-14 w-full text-white'
                  viewBox='0 0 528 40'
                  version='1.1'
                  xmlnsXlink='http://www.w3.org/1999/xlink'
                  xmlns='http://www.w3.org/2000/svg'
                >
                  <g id='shape' transform='matrix(-1.138336E-07 -1 1 -1.138336E-07 0 39.92764)'>
                    <path
                      d='M0 0L40.5467 0C40.5467 0 -31.8215 230.87 38.7134 528.217C39.8794 533.133 31.7549 527.502 31.0925 528.75C28.7914 533.084 26.1543 528.191 24.4327 529.178C59.2372 539.206 14.0091 521.981 12.9329 530.001L1.02722 528.284L0 0Z'
                      transform='translate(7.629395E-06 6.103516E-05)'
                      fill='currentColor'
                      stroke='none'
                    ></path>
                  </g>
                </svg>
              </div>
            </div>
            <div className='p-6'>
              <p className='text-sm'>13 March, 2020</p>
              <h4 className='my-2 text-lg font-semibold hover:text-blue-700'>
                <a href='/prompt_tr/landing/agency'>Top 10 design inspirations to follow</a>
              </h4>
              <p className='my-2 text-slate-400'>
                The following Italian restaurant WordPress themes come with the powerful drag-n-drop...&nbsp;
                <a className='text-slate-800 hover:text-blue-700' href='/prompt_tr/landing/agency'>
                  Read More
                </a>
              </p>
            </div>
          </div>
        </div>
      </div>
    </section>
  )
}
